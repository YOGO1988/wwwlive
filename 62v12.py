...
class ResultsGeneratorApp:
    ...
    # Tworzę funkcję do dodawania nagłówków i stopek z danymi, które będą używane przez doc.build
            def add_header_footer(canvas, doc):
                ...
                # Ścieżki do loga - określ ścieżkę względną lub bezwzględną
                script_dir = os.path.dirname(os.path.abspath(__file__))  # Katalog skryptu
                logo_path = os.path.join(script_dir, "logo.png")
                logo1_path = os.path.join(script_dir, "logo1.png")
                
                # Logo w prawym górnym rogu - sprawdź czy istnieje
                if os.path.exists(logo_path):
                    logo = Image(logo_path, width=80, height=35)  # Zmniejszone logo
                    logo.drawOn(canvas, doc.width + doc.leftMargin - 85, doc.height + doc.topMargin - 40)
                else:
                    print(f"Uwaga: Nie znaleziono logo.png w ścieżce: {logo_path}")
                
                # Dopasowanie logo1 (jeśli istnieje)
                if os.path.exists(logo1_path):
                    try:
                        logo1 = Image(logo1_path, width=80, height=35)  # Dopasowane do rozmiaru logo
                        logo1.drawOn(canvas, doc.width + doc.leftMargin - 170, doc.height + doc.topMargin - 40)
                    except Exception as e:
                        print(f"Błąd podczas dodawania logo1.png: {e}")
                else:
                    print(f"Uwaga: Nie znaleziono logo1.png w ścieżce: {logo1_path}")
                ...
                # Logo w środku stopki
                if os.path.exists(logo_path):
                    logo_stopka = Image(logo_path, width=25, height=12)
    ...
    def add_header_footer(self, canvas, doc):
        ...
        # Logo w prawym górnym rogu - jeszcze mniejsze, obniżone
        logo_path = "logo.png"
        if os.path.exists(logo_path):
            logo = Image(logo_path, width=80, height=35)  # Zmniejszone logo
            logo.drawOn(canvas, doc.width + doc.leftMargin - 85, doc.height + doc.topMargin - 40)
